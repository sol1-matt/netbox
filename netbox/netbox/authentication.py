...
class RemoteUserBackend(_RemoteUserBackend):
    ...
    def configure_groups(self, user, remote_groups):
        ...
        logger.debug(f"Configuring groups {remote_groups} for {user}")
    ...
    def authenticate(self, request, remote_user, remote_groups=None):
        ...
        if self.create_unknown_user:
            logger.debug(f"happy to create a user for {remote_user} (cleaned to {username})")
        ...
        if self.user_can_authenticate(user):
            logger.debug(f"user can auth")
            if remote_groups:
                logger.debug(f"remote_groups")
                if user is not None and not isinstance(user, AnonymousUser):
                    logger.debug(f"configuring groups")
                    return self.configure_groups(user,remote_groups)
            else:
                logger.debug(f"no remote_groups for you!")
                return user
        else:
            logger.debug(f"user cannot auth; wtf?!?")
            return None
